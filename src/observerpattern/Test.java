...
//        ConditionsObserver currentConditionsDisplay3 = new ConditionsObserver();//观察者/初始化的时候就绑定在一起了（注册）
        ConditionsObserver currentConditionsDisplay4 = new ConditionsObserver(weatherDataSubject);//观察者/初始化的时候就绑定在一起了（注册）
        currentConditionsDisplay4.removeObserver();
...
//        weatherDataSubject.setData(19f,22f,30f);//改变主体状态（数据源发送改变）
